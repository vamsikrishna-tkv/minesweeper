...
class Sentence():
    ...
    def known_mines(self):
        ...
        """
        if len(self.cells) <= self.count:  # As if cells count is less than the mine count we assume that all the cells are mines
            return set(self.cells)
        else:
            return set()  # Here we are returning empty set

    def known_safes(self):
        """
        Returns the set of all cells in self.cells known to be safe.
        """
        if self.count == 0:  # If the count is zero we assume that the cells are safe
            return set(self.cells)
        else:
            return set()
    ...
    def mark_mine(self, cell):
        ...
        """
        if cell not in self.cells:  # If the cell is not present in cells then we return noe
            return None
        else:
            self.cells.remove(cell)  # If the cell in present in objects cells we remove the cell and decrease the count
    ...
    def mark_safe(self, cell):
        ...
        if cell not in self.cells:  # If the cell is not present in cells then we return noe
            ...
            self.cells.remove(cell)  # Here we dont decrease the count because this is a safe.
...
class MinesweeperAI():
    ...
    def neighbour_cells(self, cell):
        # Retruns a set of neighbouring cells
        neighbors = set()
